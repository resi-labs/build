...
function compile_uboot_target() {
	...
	if [[ ! -f scripts/config ]]; then
		...
		echo "CONFIG_REGEX=y" >> .config
		echo "CONFIG_CMD_SETEXPR=y" >> .config
		echo "CONFIG_CMD_EXPORTENV=y" >> .config
	else
		display_alert "scripts/config found" "u-boot ${version} $BOOTCONFIG ${target_make}" "debug"

		# $BOOTDELAY can be set in board family config, ensure autoboot can be stopped even if set to 0
		if [[ $BOOTDELAY == 0 ]]; then
			display_alert "Adding CONFIG_ZERO_BOOTDELAY_CHECK=y u-boot config" "BOOTDELAY==0 for ${target}" "debug"
		...
		if [[ -n $BOOTDELAY ]]; then
			display_alert "Hacking autoboot delay in u-boot config" "BOOTDELAY=${BOOTDELAY} for ${target}" "debug"
		...
		display_alert "Hacking log level in u-boot config" "LOGLEVEL=6 for ${target}" "debug"
		run_host_command_logged scripts/config --set-val CONFIG_LOGLEVEL 6

		run_host_command_logged scripts/config --set-val CONFIG_REGEX "y"
		run_host_command_logged scripts/config --set-val CONFIG_CMD_SETEXPR "y"
		run_host_command_logged scripts/config --set-val CONFIG_CMD_EXPORTENV "y"
	fi
